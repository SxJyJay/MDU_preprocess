...
@torch.no_grad()
def add_virtual_mask(masks, labels, points, raw_points, num_virtual=200, dist_thresh=3000, num_camera=6, intrinsics=None, transforms=None, K=1):
    ...
    offsets = [] 
    for mask in masks:
        boundary_mask = mask.reshape(1, 1, W, H)
        boundary_mask = F.avg_pool2d(boundary_mask, kernel_size=3, stride=1, padding=1)
        # filter eroded background region
        boundary_mask = boundary_mask.squeeze() * mask.reshape(W, H)
        # filter inner object region
        boundary_mask = (boundary_mask > 0) * (boundary_mask < 1)
        
        indices = boundary_mask.nonzero()
...
def process_one_frame(info, predictor, data, num_camera=6):
    ...
    if len(masks) == 0:
        ...
        res  = add_virtual_mask(masks.clone(), labels.clone(), P.clone(), to_tensor(lidar_points), num_virtual=200,
            intrinsics=to_batch_tensor(all_cams_intrinsic), transforms=to_batch_tensor(all_cams_from_lidar), K=1)
...
def main(args):
    ...
    for idx, data in tqdm(enumerate(data_loader), total=len(data_loader.dataset)):
        ...
        # output_path = os.path.join(*tokens[:-2], "FOREGROUND_MIXED_BD_200", tokens[-1]+'.pkl.npy')
        output_path = os.path.join(available_root, tokens[-3], "FOREGROUND_MIXED_BD_200", tokens[-1]+'.pkl.npy')
        # import ipdb
        # ipdb.set_trace()
...
if __name__ == '__main__':
    ...
    if not os.path.isdir('/share_io02_hdd/jiaoyang/nuScenes/samples/FOREGROUND_MIXED_BD_200'):
        os.mkdir('/share_io02_hdd/jiaoyang/nuScenes/samples/FOREGROUND_MIXED_BD_200')

    if not os.path.isdir('/share_io02_hdd/jiaoyang/nuScenes/sweeps/FOREGROUND_MIXED_BD_200'):
        os.mkdir('/share_io02_hdd/jiaoyang/nuScenes/sweeps/FOREGROUND_MIXED_BD_200')
